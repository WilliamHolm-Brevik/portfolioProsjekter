...
class Ball(SpillObjekt):
    ...
    def flytt(self, brett):
        ...
        if self.xPosisjon + self.bredde >= brett.hoyde or self.xPosisjon - self.bredde <= 0:
            self.dx *= -1
...
baller = []
for i in range(3):
    ballen = Ball(rd.randint(0, brett.bredde), rd.randint(0, brett.hoyde), GRONN, brett, 20, 20, False)
    baller.append(ballen)
