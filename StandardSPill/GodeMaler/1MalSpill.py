...
class Spiller(SpillObjekt):
    ...
    def flytt(self, brett):
        ...
        elif self.fartRetning == "ned" and self.yPosisjon < brett.hoyde - self.bredde:
            self.yPosisjon += self.fart
...
# Lager spiller
spiller = Spiller(brett.bredde/2, brett.bredde/2, GRONN, brett, 20, 20, 3, 10, 10)
annetObjekt = AnnetObjekt(100, 100, GRONN, brett, 20, 20)


# Legger til spiller på brettet
brett.leggTilObjekter(spiller)
brett.leggTilObjekter(annetObjekt)
...
while fortsett:
    ...
    if annetObjekt.sjekkKollisjon(spiller):
