...
stjerner = []
for i in range(4):
    stjernen = Stjerne(rd.randint(100, brett.bredde-100), rd.randint(100, brett.bredde -100), GUL, brett, 20, 20, True)
    stjerner.append(stjernen)
    brett.leggTilObjekter(stjernen)


hindringer = []
for i in range(3):
    hindring = Hindring(rd.randint(100, brett.bredde-100), rd.randint(100, brett.bredde -100), GRAA, brett, 20, 20, True)
...
while fortsett:
    ...
    for stjernenen in stjerner:
        if spiller.sjekkKollisjon(stjernenen):
            if not Kollisjon:
                stjernenen.xPosisjon = 2000
                stjernenen.oppdaterRektangel()
                
                brett.poeng += 1
                nyStjerne = Stjerne(rd.randint(100, brett.bredde), rd.randint(100, brett.hoyde), GUL, brett, 20, 20, True)
                stjerner.append(nyStjerne)
                brett.leggTilObjekter(nyStjerne)
                nyHindring = Hindring(rd.randint(100, brett.bredde-100), rd.randint(100, brett.bredde -100), GRAA, brett, 20, 20, True)
            sjekkKollisjon = True
    if sjekkKollisjon:
        Kollisjon = True
    else:
    ...
    for hindringen in hindringer:
        if spiller.sjekkKollisjon(hindringen):
